...
const NGO_Details = () => {
  ...
  const isRegistered = (event) => {
    ...
    return volunteers.some((v) => v.user_id === "current_user_id"); 
  };
    ...
    <Box sx={{ bgcolor: "#f9f9f9", minHeight: "100vh", pt: "80px", pb: 6 }}>
      <Container maxWidth="lg">
        ...
        <Box sx={{ textAlign: "center", mb: 4 }}>
          <Typography variant="h3" sx={{ fontWeight: 700, color: "#E76F51", mb: 1 }}>
            {ngo?.name || "NGO"}
          </Typography>
        ...
        <Box sx={{ width: "100%", mb: 4, position: "relative" }}>
          <Card sx={{ width: "100%", height: { xs: 300, sm: 450, md: 500 }, borderRadius: 2, boxShadow: 3, overflow: "hidden" }}>
            ...
            <Button
              ...
              sx={{ position: "absolute", top: 20, right: 20, bgcolor: "rgba(255,255,255,0.8)", "&:hover": { bgcolor: "#fff" } }}
            >
              ...
              <CardContent>
                ...
                {ngo?.donation_link && (
                  <Box sx={{ display: "flex", gap: 2, mt: 3 }}>
                    <Button href={ngo.donation_link} target="_blank" variant="contained" sx={{ bgcolor: "#E76F51", color: "#fff", fontWeight: 700, textTransform: "none", borderRadius: "8px", py: 1.2, "&:hover": { bgcolor: "#D65A3C" } }}>
                      Visit Website
                    </Button>
                    <Button variant="contained" sx={{ bgcolor: "#E76F51", color: "#fff", fontWeight: 700, textTransform: "none", borderRadius: "8px", py: 1.2, "&:hover": { bgcolor: "#D65A3C" } }} onClick={() => setDonationModalOpen(true)}>
                      Donate
                    </Button>
                  </Box>
                )}
        ...
        <Modal open={volunteerModalOpen} onClose={() => setVolunteerModalOpen(false)}>
          <Box sx={{
            ...
            {selectedEvent && (
              <Stack spacing={2}>
                <Typography variant="h5" sx={{ fontWeight: 700, color: "#E76F51", textAlign: "center" }}>
                  {selectedEvent.title}
                </Typography>
                ...
                <Typography variant="body2">
                  Remaining Slots: {selectedEvent.volunteers_needed - (selectedEvent.volunteers?.length || 0)}
                </Typography>

                <Button
                  variant="contained"
                  sx={{ bgcolor: "#E76F51", color: "#fff", fontWeight: 700, "&:hover": { bgcolor: "#D65A3C" } }}
                  onClick={confirmVolunteer}
                  disabled={isRegistered(selectedEvent)}
                >
                  {isRegistered(selectedEvent) ? "Already Registered" : "Confirm Registration"}
                </Button>

                <Button
                  variant="outlined"
                  sx={{ borderColor: "#E76F51", color: "#E76F51", "&:hover": { borderColor: "#D65A3C", color: "#D65A3C" } }}
